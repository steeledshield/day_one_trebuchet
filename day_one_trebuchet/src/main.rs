...
//Damn elves
    ...
    //Array of calibrations from the calibration document
    let calibrations = [
    "gsjgklneight6zqfz",
    ...
    //Base number that will store the sum of each result from the for loop
    let mut num_total: i32 = 0;

    //For loop cycling through the array
    for i in calibrations.iter() {

        //converts any number stings to number values
        let word_convert = i.replace("one", "o1n1e").replace( "two", "t2w2o").replace( "three", "t3hre3e").replace( "four", "f4ou4r").replace( "five", "f5iv5e").replace( "six", "s6i6x").replace( "seven", "s7eve7n").replace( "eight", "e8igh8t").replace( "nine", "n9in9e");

        let digits: String = word_convert.chars().filter(|char| char.is_digit(10)).collect();
    
        //Slices out the first character in the string of numbers
        let  num_start: String = digits.chars().next().unwrap().to_string();
        //Slices out the last character in the string of numbers
        let num_end: String = digits.chars().last().unwrap().to_string();

        //converts the chars taken from the last step, then converts them into strings and then into values
        //Then finally gets the sum of the 2 values
        let num_sum: String = num_start + num_end.as_str();

        //adds the sum taken from each item in the array into a total sum
        num_total = num_total + num_sum.to_string().parse::<i32>().unwrap();
